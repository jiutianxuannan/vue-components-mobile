<template>
  <div class="yz-palette-button" :class="{ expand: expanded, 'yz-palette-button-active': transforming }"
    @animationend="onMainAnimationEnd" @webkitAnimationEnd="onMainAnimationEnd" @mozAnimationEnd="onMainAnimationEnd">
    <div class="yz-sub-button-container">
      <slot></slot>
    </div>
    <div @touchstart="toggle" class="yz-main-button" :style="mainButtonStyle">
      {{content}}
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    props: {
      ...
      mainButtonStyle: {
        type: String,           // 应用到 yz-main-button 上的 class
        default: ''
      }
    },
    methods: {
...
<style scoped>
  ...
  .yz-palette-button-active {
    animation: yz-zoom 0.5s ease-in-out;
  }

  .yz-sub-button-container>* {
    position: absolute;
    top: 15px;
    left: 15px;
    width: 25px;
